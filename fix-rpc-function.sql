...
-- The issue is that profiles table likely uses 'id' not 'user_id'

CREATE OR REPLACE FUNCTION get_seller_purchase_requests_fixed(
    p_seller_id UUID,
...
BEGIN
    ...
    JOIN profiles p ON pr.buyer_id = p.id  -- Changed from p.user_id to p.id
