-- ⚔️ HSTNLX DISTRIBUTED TRANSACTION ENGINE (V12.2)
-- Institutional-grade cart processing and payment reconciliation.
...
BEGIN
  ...
  LOOP
    ...
    LOOP
       ...
       -- Fetch actual price from DB (Hard Truth)
       SELECT price INTO v_db_price FROM public.products WHERE id = v_p_id FOR SHARE;
       
       RAISE NOTICE 'HSTNLX TRANSACTION: Processing Product % with Qty %', v_p_id, v_p_qty;
