-- ⚔️ HSTNLX DUAL-CHANNEL HARDENED ENGINE (V13.2)
-- Channels: Browser (auth.uid()) & Webhook (Service Role Override)
...
CREATE OR REPLACE FUNCTION public.place_order_after_payment(
  ...
  p_shipping JSONB,
  p_user_id_override UUID DEFAULT NULL -- Only used by Webhook Authority
)
RETURNS UUID AS $$
DECLARE
  v_user_id UUID;
...
BEGIN
  ...
  LOOP
    ...
    VALUES ('order_created', 'dist_engine_v13.2', 'success', v_user_id, v_order_id::text, jsonb_build_object('payment_id', p_payment_id, 'amount', v_order_total, 'channel', CASE WHEN auth.uid() IS NULL THEN 'webhook' ELSE 'browser' END));
